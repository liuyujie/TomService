...
- (void)send:(TOMMessageModel *)rootMsg seq:(UInt32)s callback:(TCPBlock)block {
    ...
    if ((self.loginStatus == false || [[SocketClientManager instance] status] == false)
        && self.autoLogin == true
        && rootMsg != nil && rootMsg.type != TOMMessageTypeLogin) {
        NSLog(@"进入自动登录");
    ...
    // 打包
    if (rootMsg == nil && rootMsg.type == TOMMessageTypePing) {   // 心跳包没 body
        SInt32 length = 0;
...
- (void)requestLogin:(NSString *)name password:(NSString *)psw completion:(TCPBlock)block {
    dispatch_async(self.APIQueue, ^{
        ...
        // 如果登录了，先下线
        if ([[SocketClientManager instance] status] == true) {
